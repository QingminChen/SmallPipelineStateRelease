...
resource "google_storage_bucket" "restore_state_bucket" {
  name = "terraform_state_colletion"
  location = "us-central1"
  project = "dataprod-cluster-testing-1"
  storage_class = "STANDARD"
  bucket_policy_only = true
...
resource "google_storage_bucket_object" "upload-state-file" {     //used
  ...
  provisioner "local-exec" {
    ...
    command = "gsutil cp -r ${var.root-terraform-project-home-folder}/main.tf ${var.root-small-deploy-source-code-bucket}/terraform_state_restore"
  }
